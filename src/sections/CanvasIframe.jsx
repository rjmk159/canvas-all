...
export default function CanvasIframe() {
  ...
  useEffect(() => {
    const onMsg = (e) => {
      if (e.data && e.data.type === 'iframe-canvas-click') {
        setInnerClicks(e.data.clicks)
      }
    }
  ...
  return (
    <div>
      ...
      <p className="section-sub">
        An animated 2D canvas runs in a same-origin <span className="kbd">/iframe-canvas.html</span> page. Clicks inside the frame are reported back to the parent via postMessage.
      </p>
      <ScreenNav
        screens={SCREENS}
        active={screen}
      ...
      <div className="row" style={{ marginBottom: 12 }}>
        <span className="badge" data-testid="iframe-click-count">inner clicks: {innerClicks}</span>
